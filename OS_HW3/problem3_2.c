...
static pthread_mutex_t group_m;
static pthread_cond_t run_cv;
...
runners_life(void *data)
{
    ...
    while (runner->late < LATE_THRESHOLD
        ...
        if(runner->state == RUNNER_ASSEMBLING)
        {
            ...
            {
                pthread_cond_wait(&run_cv, &group_m);
            }
            pthread_mutex_unlock(&group_m);
        }
        else
...
main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        void *pointer = &runner[i];
        /* XXX create thread for runner[i] XXX */
        if(pthread_create(&runner[i].tid, NULL, runners_life, pointer))
        {
            (void) fprintf(stderr, "Error creating thread\n");
            rc = EXIT_FAILURE;
            goto cleanup;
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        ...
        if(pthread_join(runner[i].tid, NULL))
        {
            (void) fprintf(stderr, "Error joining threads\n");
            rc = EXIT_FAILURE;
            goto cleanup;
        }
